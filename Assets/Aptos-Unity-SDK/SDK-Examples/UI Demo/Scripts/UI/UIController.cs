...
namespace Aptos.Unity.Sample.UI
{
    public class UIController : MonoBehaviour
    {
        ...
        [SerializeField] private PanelTab gameShopTab;
        [SerializeField] private PanelTab playGameTab;
        ...
        void InitStatusCheck()
        {
            ...
            if (PlayerPrefs.GetString(AptosUILink.Instance.mnemonicsKey) != string.Empty)
            {
                ...
                //Check NFT Balance here
                Debug.Log("CheckGameNFT " + PlayerPrefs.GetString(AptosUILink.Instance.mnemonicsKey));
                StartCoroutine(AptosUILink.Instance.CheckGameNFT());
            }
            else
            {
                //Reset all data tại đây
        ...
        public void ToggleEmptyState(bool _empty)
        {
            ...
            gameShopTab.DeActive(_empty);
            playGameTab.DeActive(_empty);
        ...
        void UpdateBalance(float _amount)
        {
            ...
            aptosBalance = AptosUILink.Instance.AptosTokenToFloat(_amount);
        }
        ...
        public void SetNetwork(TMP_Dropdown _target)
        {
            ...
            RestClient.Instance.SetEndPoint(Constants.TESTNET_BASE_URL);
            ToggleNotification(ResponseInfo.Status.Success, "Set Network to Testnet");
        }
        ...
        {
            StartCoroutine(AptosUILink.Instance.ClaimGameNFT(index));
        }
        ...
        public void BuyGameGoldAndGemUI(int index)
        {
            StartCoroutine(AptosUILink.Instance.BuyGameGoldAndGem(index));
        }
